...
/*   Updated: 2024/03/12 16:46:30 by aaaraba          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init(t_all *all)
{
	...
	all->r_img = mlx_xpm_file_to_image(all->mlx,
			"g/route.xpm", &width, &height);
	all->wall_img = mlx_xpm_file_to_image(all->mlx,
			"g/wall.xpm", &width, &height);
	all->exit_img = mlx_xpm_file_to_image(all->mlx,
			"g/Door.xpm", &width, &height);
	all->coin_img = mlx_xpm_file_to_image(all->mlx,
			"g/coin.xpm", &width, &height);
	return (ft_protect(all));
}

void	ft_change_imgs(t_all *all, t_size *size)
{
	if (all->map_2d[size->p_y][size->p_x] == 'E' && size->coin <= 0)
	{
		mlx_put_image_to_window(all->mlx,
			all->win, all->r_img, size->x * 50, size->y * 50);
		mlx_put_image_to_window(all->mlx,
			all->win, all->exit_img, size->p_x * 50, size->p_y * 50);
		exit(0);
	}
	else
	{
		if (all->map_2d[size->p_y][size->p_x] == 'C')
		{
			all->map_2d[size->p_y][size->p_x] = '0';
			size->coin--;
		}
